...
def run_simple_spread_aec_with_gif(gif_filename="simple_spread_episode.gif", num_episodes=1, fps=20):
    print("Starting AEC version of simple_spread script with GIF saving.")
    frames = []

    try:
        env = simple_spread_v3.env(N=3, local_ratio=0.5, max_cycles=25, render_mode="rgb_array", continuous_actions=True)
        print("Environment created successfully with render_mode='rgb_array'.")
        env.reset()
        print("Environment reset successfully.")
    except Exception as e:
        print(f"Error initializing environment: {e}")
    ...
    print("Environment initialized for GIF capture.")
    print(f"Agents in environment: {env.agents}")
    ...
    for episode in range(num_episodes):
        print(f"\n=== Episode {episode + 1} (for GIF capture) ===")
        env.reset()
        ...
        for agent in env.agent_iter():
            ...
            # Capture frame after each agent's step
            frames.append(env.render())
    ...
    try:
        ...
        print("Make sure you have imageio and imageio[ffmpeg] installed.")

    print("Script finished.")


# Call the function to run the simulation and save the GIF
if __name__ == "__main__":
    # You can choose to run the original script (no GIF) or the one that saves a GIF
    # run_simple_spread_aec_script() 
    run_simple_spread_aec_with_gif(gif_filename="simple_spread_simulation.gif", num_episodes=1, fps=20)